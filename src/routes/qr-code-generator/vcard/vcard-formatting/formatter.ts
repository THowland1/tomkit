import { vCardAddressType, type vCardAddress, type vCardFormattingOptions } from './types';
...
export class vCardFormatter {
		...
		if (
			...
			if (majorVersion >= 4) {
				formattedAddress =
					...
					';' +
					vCardFormatter.e(address.details.countryRegion) +
					vCardFormatter.nl();
			} else {
				if (address.details.label) {
					formattedAddress =
						...
						address.type +
						':' +
						vCardFormatter.e(address.details.label) +
						vCardFormatter.nl();
				}
				formattedAddress +=
					'ADR' +
	...
	public static getFormattedString(vCard: vCard, options?: vCardFormattingOptions): string {
		...
		formattedVCardString +=
			'FN' + encodingPrefix + ':' + vCardFormatter.e(formattedName) + vCardFormatter.nl();
		formattedVCardString +=
			'N' +
			encodingPrefix +
			':' +
			...
			';' +
			vCardFormatter.e(vCard.nameSuffix) +
			vCardFormatter.nl();

		if (vCard.nickname && majorVersion >= 3) {
			formattedVCardString +=
				'NICKNAME' + encodingPrefix + ':' + vCardFormatter.e(vCard.nickname) + vCardFormatter.nl();
		}
		...
		if (vCard.uid) {
			formattedVCardString +=
				'UID' + encodingPrefix + ':' + vCardFormatter.e(vCard.uid) + vCardFormatter.nl();
		}

		if (vCard.birthday) {
			formattedVCardString +=
				'BDAY:' + vCardFormatter.getFormattedDate(vCard.birthday) + vCardFormatter.nl();
		}

		if (vCard.anniversary) {
			formattedVCardString +=
				'ANNIVERSARY:' + vCardFormatter.getFormattedDate(vCard.anniversary) + vCardFormatter.nl();
		}
		...
		if (vCard.cellPhone) {
			...
			vCard.cellPhone.forEach(function (number) {
				if (majorVersion >= 4) {
					formattedVCardString +=
						'TEL;VALUE=uri;TYPE="voice,cell":tel:' + vCardFormatter.e(number) + vCardFormatter.nl();
				} else {
		...
		if (vCard.pagerPhone) {
			...
			vCard.pagerPhone.forEach(function (number) {
				if (majorVersion >= 4) {
					formattedVCardString +=
						'TEL;VALUE=uri;TYPE="pager,cell":tel:' + vCardFormatter.e(number) + vCardFormatter.nl();
				} else {
					formattedVCardString +=
						'TEL;TYPE=PAGER:' + vCardFormatter.e(number) + vCardFormatter.nl();
				}
			});
		}
		...
		if (vCard.homePhone) {
			...
			vCard.homePhone.forEach(function (number) {
				if (majorVersion >= 4) {
					formattedVCardString +=
						'TEL;VALUE=uri;TYPE="voice,home":tel:' + vCardFormatter.e(number) + vCardFormatter.nl();
				} else {
					formattedVCardString +=
						'TEL;TYPE=HOME,VOICE:' + vCardFormatter.e(number) + vCardFormatter.nl();
				}
			});
		}
		...
		if (vCard.workPhone) {
			...
			vCard.workPhone.forEach(function (number) {
				if (majorVersion >= 4) {
					formattedVCardString +=
						'TEL;VALUE=uri;TYPE="voice,work":tel:' + vCardFormatter.e(number) + vCardFormatter.nl();
				} else {
					formattedVCardString +=
						'TEL;TYPE=WORK,VOICE:' + vCardFormatter.e(number) + vCardFormatter.nl();
				}
			});
		}
		...
		if (vCard.homeFax) {
			...
			vCard.homeFax.forEach(function (number) {
				if (majorVersion >= 4) {
					formattedVCardString +=
						'TEL;VALUE=uri;TYPE="fax,home":tel:' + vCardFormatter.e(number) + vCardFormatter.nl();
				} else {
					formattedVCardString +=
						'TEL;TYPE=HOME,FAX:' + vCardFormatter.e(number) + vCardFormatter.nl();
				}
			});
		}
		...
		if (vCard.workFax) {
			...
			vCard.workFax.forEach(function (number) {
				if (majorVersion >= 4) {
					formattedVCardString +=
						'TEL;VALUE=uri;TYPE="fax,work":tel:' + vCardFormatter.e(number) + vCardFormatter.nl();
				} else {
					formattedVCardString +=
						'TEL;TYPE=WORK,FAX:' + vCardFormatter.e(number) + vCardFormatter.nl();
				}
			});
		}
			...
			vCard.otherPhone.forEach(function (number) {
				if (majorVersion >= 4) {
					formattedVCardString +=
						...
						vCardFormatter.e(number) +
						vCardFormatter.nl();
				} else {
					formattedVCardString +=
						'TEL;TYPE=OTHER:' + vCardFormatter.e(number) + vCardFormatter.nl();
				}
			});
		}
		...
		if (vCard.title) {
			formattedVCardString +=
				'TITLE' + encodingPrefix + ':' + vCardFormatter.e(vCard.title) + vCardFormatter.nl();
		}

		if (vCard.role) {
			formattedVCardString +=
				'ROLE' + encodingPrefix + ':' + vCardFormatter.e(vCard.role) + vCardFormatter.nl();
		}

		if (vCard.organization) {
			formattedVCardString +=
				'ORG' + encodingPrefix + ':' + vCardFormatter.e(vCard.organization) + vCardFormatter.nl();
		}

		if (vCard.url) {
			formattedVCardString +=
				'URL' + encodingPrefix + ':' + vCardFormatter.e(vCard.url) + vCardFormatter.nl();
		}
		...
		if (vCard.workUrl) {
			formattedVCardString +=
				...
				encodingPrefix +
				':' +
				vCardFormatter.e(vCard.workUrl) +
				vCardFormatter.nl();
		}

		if (vCard.note) {
			formattedVCardString +=
				'NOTE' + encodingPrefix + ':' + vCardFormatter.e(vCard.note) + vCardFormatter.nl();
		}

		if (vCard.socialUrls) {
			for (const key of vCard.socialUrls.keys()) {
				if (vCard.socialUrls.has(key)) {
					formattedVCardString +=
						'X-SOCIALPROFILE;TYPE=' +
						key +
						':' +
						vCardFormatter.e(vCard.socialUrls.get(key)) +
						vCardFormatter.nl();
				}
			}
		}
		...
		if (vCard.source) {
			formattedVCardString +=
				'SOURCE' + encodingPrefix + ':' + vCardFormatter.e(vCard.source) + vCardFormatter.nl();
		}
